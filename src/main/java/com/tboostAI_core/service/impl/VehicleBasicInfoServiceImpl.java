...
@Slf4j
@Service
public class VehicleBasicInfoServiceImpl implements VehicleBasicInfoService {

    private final VehicleRepo vehicleRepo;
    private final GoogleGeocodingServiceImpl googleGeocodingServiceImpl;
    ...
    @Autowired
    public VehicleBasicInfoServiceImpl(VehicleRepo vehicleRepo, GoogleGeocodingServiceImpl googleGeocodingServiceImpl, WebClientUtils webClientUtils, RedisServiceForOpenAIImpl redisServiceForOpenAIImpl) {
        this.vehicleRepo = vehicleRepo;
        this.googleGeocodingServiceImpl = googleGeocodingServiceImpl;
        this.webClientUtils = webClientUtils;
        this.redisServiceForOpenAIImpl = redisServiceForOpenAIImpl;
        this.objectMapper = new ObjectMapper();
    }

    private static final Logger logger = LoggerFactory.getLogger(VehicleBasicInfoServiceImpl.class);
    ...
    private void relaxSearchParams(SearchVehicleListRequest searchVehicleListRequest) {
        ...
        logger.info("VehicleBasicInfoServiceImpl: relaxSearchParams - Relax Search Vehicle list request: {}", searchVehicleListRequest);
    }
    ...
    private SearchVehiclesResponse executeSearch(SearchVehicleListRequest searchVehicleListRequest, Pageable pageable) {
        Specification<VehicleBasicInfoEntity> spec = (root, query, cb) -> {
            ...
            if (searchVehicleListRequest.getDistance() != null && searchVehicleListRequest.getDistance() > 0) {
                ...
                logger.info("VehicleBasicInfoServiceImpl: executeSearch - Search distance is : {}", maxDistanceInMeters);
            }
    ...
    public SearchVehiclesResponse searchVehiclesByLLM(String sessionId, Double minPrice, Double maxPrice, List<String> bodyType, List<String> engineType, String content, String address, int distance, Pageable pageable) {
        ...
        return result.responseObj().map(SearchParamsMapper.INSTANCE::mapWithDefaultValues).flatMap(processedRequest -> {
            try {
                ...
                redisServiceForOpenAIImpl.saveMessageToList(sessionId, result.newUserMessage());
